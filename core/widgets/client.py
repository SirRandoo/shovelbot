...
from core.utils import commands, dataclasses
...
class ShovelBot(QtWidgets.QMainWindow):
    ...
    # The aboutTo_____ signals are emitted before the client advances in its
    # lifecycle.  While you can call other lifecycle events, its not advised
    # as it can confuse the end-user, and internally the lifecycle is expected
    # to follow the following:
    # Start > Halt > Stop
    # Start > Stop
    # Start > Halt > Start > (any other cycles listed here)
    aboutToStart = QtCore.pyqtSignal()
    aboutToHalt = QtCore.pyqtSignal()
    aboutToStop = QtCore.pyqtSignal()
    
    # These signals are emitted after the client has finished a lifecycle event.
    ...
    def __init__(self):
        ...
        self.extensions: Dict[str, Union[dataclasses.ExtensionStub, dataclasses.Extension]] = {}
        ...
        self.help_engine: QtHelp.QHelpEngine = QtHelp.QHelpEngineCore('resources/docs/shovelbot.qhc')
        self.command_manager: commands.Manager = commands.Manager()
        self.database: QtSql.QSqlDatabase = QtSql.QSqlDatabase.addDatabase('QSQLITE')
    ...
    def setup_help_ui(self):
        ...
        if should_create_widget(self.help_dialog):
                    ...
                    try:
                        ...
                        for raw_setting in decoded_text:
                            try:
                                ...
                                if should_create_widget(self.settings):
    ...
    def setup_extensions(self):
        ...
        for ext in self.extensions.values():
            if isinstance(ext, dataclasses.ExtensionStub):
    ...
    def unload_extensions(self):
        ...
        for name, value in self.extensions.copy().items():
            if isinstance(value, dataclasses.ExtensionStub):
            ...
            try:
                ...
                try:
                    ...
                    self.LOGGER.debug('Extensions table: {}'.format('DELETED' if should_create_widget() else 'EXISTS'))
    ...
    def load_extension(self, path: pathlib.Path) -> typing.List[dataclasses.Extension]:
        ...
        try:
            ...
            for attr, inst in query:
                ...
                if any([c == dataclasses.Extension for c in inspect.getmro(inst)]):
                    logger.debug(f'Found class {attr}!  Creating a new instance...')
                    instance: dataclasses.Extension = inst()
                    ...
                    if isinstance(instance, dataclasses.Platform):
                        ...
                        instance.onMessage.connect(lambda x: self.process_chat_message(instance, x))
    ...
    def load_from_stub(self, stub: dataclasses.ExtensionStub) -> typing.List[dataclasses.Extension]:
        """Loads an extension from an extension stub."""
        # Instance validation
        if not isinstance(stub, dataclasses.ExtensionStub):
    ...
    def about(self):
        ...
        dialog.set_caption(textwrap.dedent(f"""
            ...
            <p style="padding: 0; margin-top: 0; margin-bottom: 0;">
                ...
                Qt Version: <a href="https://www.qt.io/">{QtCore.QT_VERSION_STR}</a><br/>
