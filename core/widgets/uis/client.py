...
from core.utils import dataclasses, enums
...
class Client:
    ...
    def update_extension_display(self):
        ...
        for extension in app.client.extensions:
            ...
            # Instance check
            if isinstance(extension, dataclasses.Extension):
                # True population
    ...
    def extension_context_requested(self, point: QtCore.QPoint):
        ...
        # Instance check
        if isinstance(ext, dataclasses.Extension):
            state = ext.STATE  # Update the state to the extension's true state
        
        # State check
        if state == enums.ExtensionStates.UNLOADED:
            self.extension_load.setText('Load')  # You can only load an unloaded extension
            self.extension_unload.setDisabled(True)  # You can't unload an unloaded extension
        
        else:
            self.extension_load.setText('Reload')  # You can reload a loaded extension
    ...
    def extension_context_load(self):
        ...
        for selection in selected:
            ...
            if name not in app.client.extensions:
                ...
                stub: dataclasses.ExtensionStub = app.client.extensions[name]
            
            # Sequence check
            if isinstance(stub, dataclasses.Extension):  # Instance check
                if self.extension_load.text().lower() != 'reload':
                    return self.LOGGER.warning(f"Extension {stub.DISPLAY_NAME} is already loaded!")
                
                stub: dataclasses.Extension
    ...
    def extension_context_unload(self):
        ...
        for selection in selected:
            ...
            if name not in app.client.extensions:
                ...
                extension: dataclasses.Extension = app.client.extensions[name]
            
            # Stub instance check
            if isinstance(extension, dataclasses.ExtensionStub):
    ...
    def extension_context_toggle(self):
        ...
        # Update the QAction
        self.extension_visibility.setText('{} unloaded'.format('Show' if checked else 'Hide'))
        ...
        for key, ext in app.client.extensions.items():
            try:
                ...
                if checked:
                    if isinstance(ext, dataclasses.ExtensionStub) or ext.STATE == enums.ExtensionStates.UNLOADED:
    ...
    def extension_context_details(self):
        ...
        for selection in selected:
            ...
            if name not in app.client.extensions:
                ...
                ext: dataclasses.Extension = app.client.extensions[name]
