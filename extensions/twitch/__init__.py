...
class Twitch(core_dataclasses.Platform, core_dataclasses.Modifier, client.Client):
    ...
    def __post_init__(self, parent: QtCore.QObject = None):
        ...
        app: 'custom.QApplication' = QtWidgets.QApplication.instance()
        
        # Public attributes
        self.irc = gateway.Gateway()
        self.http = http.Http(factory=app.client.request_factory)
    ...
    def register_converters(self):
        ...
        for name, inst in inspect.getmembers(converters):
            ...
            if inspect.isfunction(inst):
                if c in self.bot.settings.converters:
                    self.LOGGER.warning(f'Converter {c} is already registered!  Overwriting...')
                    container = inspect.getmodule(self.bot.settings.converters[c])
                ...
                self.bot.settings.converters[c] = inst
    ...
    def register_settings(self):
        ...
        if 'twitch' not in self.bot.settings['extensions']:
    ...
    def stitch_settings(self):
        ...
        # Account settings
        self.bot.settings['extensions']['twitch']['token'].value_changed.connect(self.TOKEN_CHANGED.emit)
        
        self.bot.settings['extensions']['twitch']['token'].value_changed.connect(self.sync_settings)
        self.bot.settings['extensions']['twitch']['client_id'].value_changed.connect(self.sync_settings)
        self.bot.settings['extensions']['twitch']['channel'].value_changed.connect(self.sync_settings)
    ...
    def set_token(self, token: twitch_dataclasses.Token):
        ...
        self.bot.settings['extensions']['twitch']['account']['token'].set_value(token.token)
        self.bot.settings['extensions']['twitch']['account']['token'].data['scopes'] = [s.value for s in token.scopes]
        
        # Set the QLineEdit's text to the token
        try:
            view = self.bot.settings.view['extensions/twitch/account/token']
            ...
            for child in view.display.layout().children():
                if isinstance(child, QtWidgets.QLineEdit):
                    return child.setText(token.token)
    ...
    def sync_settings(self):
        ...
        channel = self.bot.settings['extensions']['twitch']['channel'].value
        token = self.bot.settings['extensions']['twitch']['token'].value
        login = None
        
        self.http.token = token
        self.http.client_id = self.bot.settings['extensions']['twitch']['client_id'].value
    ...
    # Modifier overrides
    def should_modify(self, platform: core_dataclasses.Platform) -> bool:
        """An override to ensure the Twitch extension only modifies messages
        from itself."""
        return isinstance(platform, self)
    
    def modify(self, message: core_dataclasses.Message) -> core_dataclasses.Message:
