...
class Generator(QtWidgets.QDialog):
    ...
    # Signals
    GENERATED = QtCore.pyqtSignal(object)  # token.Token
    ...
    def setup_ui(self):
        ...
        # Stacks
        if should_create_widget(self.browser_container):
            self.browser_container = QtWidgets.QWidget()
        
        if should_create_widget(self.scope_container):
            self.scope_container = QtWidgets.QWidget()
        
        # Browser widgets
        if should_create_widget(self.browser):
            self.browser = QtWebEngineWidgets.QWebEngineView()
            self.browser.urlChanged.connect(self.url_watcher)
        
        if should_create_widget(self.label):
            self.label = QtWidgets.QLabel()
            self.label.setWordWrap(True)
        
        # Scope widgets
        if should_create_widget(self.scopes):
        ...
        if should_create_widget(self.generate):
